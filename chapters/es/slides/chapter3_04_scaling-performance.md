...
`nlp.pipe` es un generador que usa `yield` para devolver objetos `Doc`, así que para obtener una lista de `Doc`, recuerda llamar la función `list` alrededor de él.
...
Así es como lo hace spaCy tras bastidores: `nlp.make_doc` convierte el texto en un doc antes de llamar a los componentes del pipeline.
