def test():
    ...
    assert words == ["Oh", ",", "really", "?", "!"], "Vérifie bien les mots !"
    assert all(isinstance(s, bool) for s in spaces), "Les espaces doivent être des booléens."
    assert [int(s) for s in spaces] == [0, 1, 0, 0, 0], "Les espaces sont-ils corrects ?"
    assert doc.text == "Oh, really?!", "Es-tu certain d'avoir créé le Doc correctement ?"
